...
const InvestorContacts = () => {
  ...
  useEffect(() => {
    fetchInvestors(); // Call the debounced function
  }, [filters, token]); // Run when filters or token changes
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-4 md:mt-0 flex flex-wrap gap-4">
            <select
              ...
              className={`rounded-md border-gray-300 py-2 pl-3 pr-10 text-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 ${
                darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white text-gray-900'
              }`}
            >
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {investors.map((investor) => (
            <div key={investor._id} className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg overflow-hidden`}>
              <div className="p-6">
                ...
                <p className={`mt-4 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{investor.description}</p>

                <div className="mt-4">
                  <h4 className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Industry Focus</h4>
                ...
                <div className="mt-4">
                  <h4 className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Investment Range</h4>
        ...
        {investors.length === 0 && (
          <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg p-8 text-center`}>
            <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>No investors found matching your filters.</p>
          </div>
        )}
      </div>
    </div>
